...
def landsat_ndvi(landsat_files, out_dir):
    ...
    NIR = arcpy.Raster( landsat_files["B4"] )
    R = arcpy.Raster( landsat_files["B3"] )
...
def cal_T6(tm5_dir, landsat_files, out_dir):
    out_fp = os.path.join(out_dir, "T6.tif")

    if os.path.exists(out_fp):
        return out_fp

    DN = arcpy.Raster(landsat_files["B6"])
    M = float(get_str_from_meta(tm5_dir, "RADIANCE_MULT_BAND_6 = (.*)"))
    A = float(get_str_from_meta(tm5_dir, "RADIANCE_ADD_BAND_6 = (.*)"))
    L = A + M * DN

    #L = 1.18243 + 0.055375 * DN

    k1 = float( get_str_from_meta(tm5_dir, "K1_CONSTANT_BAND_6 = (.*)") )
    k2 = float( get_str_from_meta(tm5_dir, "K2_CONSTANT_BAND_6 = (.*)") )
    T6 = k2 /( Ln(1+k1/L))
    #T6 = 1260.56 / (Ln(1 + 607.76 / L))
    T6.save(out_fp)
    return out_fp
...
if __name__ == '__main__':
    config = {
        "TM5Dir": r"F:\lu\19990203\LT05_L1TP_119043_19990203_20161219_01_T1",
        "clip_fp" : r"F:\lu\xiamen\xm-shapefile\xmbj.shp",
        "class_tif" : r"F:\lu\20001206\class\20001206cj_class_mj.dat",  #监督分类结果：1为建筑物、2为自然表面、3为水体
        "f" : 0.66,          #大气透射率
        "T0" : 287.15,           #温度
        "region" : 2,        #研究区：1中纬度夏季大气，2中纬度冬季大气，3热带地区，4美国1976标准大气公式
        "project_dir": r"F:\lu\19990203"
    }
